  ...
  const handleLogin = (email, password) => {
    ...
    signInWithEmailAndPassword(auth, email, password)
      .then((userCredential) => {
        ...
        onValue(ref(database, `users/${userCredential.user.uid}`), (snapshot) => {
          const userSessionId = snapshot.val().sessionId;
  ...
  return (
    <div>
      {user ? (
          ...
          <button className="red-logout-button" onClick={handleLogout}> Logout
        </button>&nbsp;&nbsp;&nbsp;&nbsp;
        <img
          style={{ height: 50 }}
